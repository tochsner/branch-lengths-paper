...
Sampling from the distribution is achieved by drawing samples from the embedding space and mapping them back to the original height space using the inverse transformation. To assess if a given time tree is within the smallest 95 \% credible region, we can generate a large number of samples from the fitted distribution. If the model probability of the given tree ranks within the top 95 \% of the sampled tree probabilities, it is in the smallest credible region.

The final operation is the construction of a point estimate based on a fitted tractable time tree distribution. The obvious choice is the highest-probability tree, as it is done for the CCD distributions \cite{ccd}. For distributions based on gamma or weibull distributions, this can lead to a tree with branch lengths of $0$. For these distributions, we choose the expected tree instead.
...
To assess the performance on biological datasets, we obtained 58 posterior time tree distributions from Dryad \cite{dryad}. Specifically, we filtered the database for files with a \emph{.trees} extension and manually selected those containing a posterior sample of ultrametric time trees obtained by BEAST 2 or a similar software. An exhaustive list of the studies considered and potential reasons for exclusion are provided in the supporting information (see also table \ref{table-bio-datasets}).
...
\begin{table}[h]
	...
	\begin{tabular}{@{}ll@{}}
		...
		Number of datasets on eukaryotes & 32 \\
